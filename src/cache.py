        ...
        try:
            ...
            try:
                ...
                remote_image = RemoteImage(
                    url=url, width=default.Width, height=default.Height)
                self.download_queue.put(remote_image)
                logging.info(
                    "Could not find image {0} in cache, added to download queue".format(url))
                return default
        ...
        while True:
            ...
            if r.status_code == 200:
                image = self.save_image(
                    digest, r.content, r.headers['Content-Type'], (remote_image.width, remote_image.height))
                self.set(digest, image)
            else:
