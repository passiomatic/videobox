...
DOWNLOADS = [
    DownloadMock(state="dowloading", name="Some release name", progress=70,
                 peers_count=99, download_speed=3.5, upload_speed=0.9),
    DownloadMock(state="dowloading", name="Other release name",
                 progress=0, peers_count=59, download_speed=9.5, upload_speed=5.9),
    DownloadMock(state="dowloading", name="A release name", progress=20,
                 peers_count=9, download_speed=10.5, upload_speed=4)
]
...
class MainWindow(wx.Frame):
    ...
    def __init__(self, app, *args, **kwargs):
        ...
        home_view = views.home.HomeView(
            self.app.image_cache, featured_series, new_series, running_series)
    ...
    def UpdateDownloadsPanel(self):
        ...
        if self.app.torrenter.torrents_status:
            ...
            downloads_view = views.downloads.DownloadsView(
                self.app.torrenter.torrents_status)
    ...
    def SetupToolBar(self, parent):
        ...
        toolbar.AddTool(ID_MENU_SYNC, label="Sync", bitmap=self.icons.sync,
                        shortHelp="Synchronize library with latest shows")
        toolbar.Realize()
        return toolbar
    ...
    def SetupMenuBar(self):
        ...
        syncItem = libraryMenu.Append(
            ID_MENU_SYNC, 'Sync', 'Synchronize library with latest shows')
...
class VideoboxApp(wx.App):
    def OnInit(self):
        ...
        self.frame = MainWindow(
            self, parent=None, id=wx.ID_ANY, title=self.AppName)
    ...
    def SyncData(self):
        if self.IsSyncing():
            ...
            self.sync_worker = sync.SyncWorker(
                progress_callback=self.OnSyncProgress, done_callback=self.SyncEnded)
