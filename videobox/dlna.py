...
# Browse response has ns1:class vs upnp:class wo this and is unusable!
ElementTree.register_namespace('upnp', UPNP_NS)
...
class BaseItem(abc.ABC):
    ...
    def __init__(self, object_id: str, path: str, urlbase: str, parent: Optional['BaseItem'], mime_type=''):
        ...
        self._date = datetime.datetime.fromtimestamp(
            os.stat(self._file_path).st_mtime)
        self._cover = None
        self._captions = None
    ...
    def to_element(self, object_id: str, browse_direct_children: bool):
        root = ElementTree.Element(self._element_name(), {
                                   'id': self._object_id})
        if self._parent is not None:
            root.attrib['parentID'] = self._parent.get_id()

        ElementTree.SubElement(root, ElementTree.QName(
            UPNP_NS, 'class')).text = self._upnp_class()
        dc_ns = 'http://purl.org/dc/elements/1.1/'
        ElementTree.SubElement(root, ElementTree.QName(
            dc_ns, 'title')).text = self.get_name()
        ElementTree.SubElement(root, ElementTree.QName(
            dc_ns, 'date')).text = self._date.isoformat()
    ...
    def res_element(url, mime_type, size, extra_dlna_information='*'):
        root = ElementTree.Element('res', {
                                   'protocolInfo': f'http-get:*:{mime_type}:{extra_dlna_information}', 'size': str(size)})
        root.text = url
        return root
...
class VideoItem(BaseItem):
    ...
    def to_element(self, object_id: str, browse_direct_children: bool):
        ...
        if self._cover:
            ...
            root.append(self.res_element(
                cover, mime_type, size, 'DLNA.ORG_PN=JPEG_TN'))
...
class DirectoryItem(BaseItem):
    # All items except the root should have a parent
    def __init__(self, object_id: str, path: str, urlbase: str, parent: Optional[BaseItem]):
...
class Content:
    ...
    def _get_id(self, path):
        encode_path = self._content_dir if path == self._content_dir else path.replace(
            self._content_dir, '')
    ...
    def _add_item(self, path: str, parent: Optional[BaseItem]):
        ...
        # TODO: search for .mp4 files only..vs checking mime types
...
class SSDPServer(asyncio.DatagramProtocol):
    ...
    def _make_notify_message(self, usn, sub_type: str):
        ...
        parts = ['NOTIFY * HTTP/1.1', f'HOST: {SSDPServer.ADDRESS}:{SSDPServer.PORT}', f'{SSDPServer.Header.NTS.value}: {sub_type}']
        return self._make_message(parts, data)

    def _send_alive(self, usn):
        data = self._make_notify_message(usn, SSDPServer.Messages.ALIVE.value)
        self._send(data, (SSDPServer.ADDRESS, SSDPServer.PORT))

    def _send_bye(self, usn):
        data = self._make_notify_message(usn, SSDPServer.Messages.BYE.value)
        self._send(data, (SSDPServer.ADDRESS, SSDPServer.PORT))
...
def main():
    ...
    logging.getLogger('werkzeug').setLevel(
        logging.ERROR)  # Disable flask request logging
    ...
    if not args.port:
        ...
        sock.close()
    server = VideoServer(args.host, args.port,
                         args.content_dir, args.device_name)
