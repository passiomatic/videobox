...
class Syncer(object):
    ...
    def sync(self):
        ...
        if last_log:
            logging.info("Last sync done at {0} UTC, requesting updates since then".format(last_log.timestamp.isoformat()))
            #dialog.create(self.addon.name, 'Getting updated series...')
            response = self.do_request(lambda: api.get_updated_series(self.client_id, last_log.timestamp))
        else:
            logging.info("No previous sync found, starting a full import")
            #dialog.create(self.addon.name, 'First run: import running series...')
            response = self.do_request(lambda: api.get_running_series(self.client_id)) 
        ...
        if series_ids:
            logging.debug("Got {0} series, starting sync".format(len(series_ids)))
        ...
        if episode_ids:
            logging.debug("Got {0} episodes, starting sync".format(len(episode_ids)))            
        ...
        if any([series_ids, episode_ids, release_ids]):
            ...
            description = "Finished sync in {:.2f}s: no updates found".format(time.time()-start)
    ...
    def sync_series(self, remote_ids, dialog):
        ...
        if new_ids:
            ...
            logging.debug("Found missing {0} of {1} series".format(new_ids_count, len(remote_ids)))           
            response = self.do_chunked_request(api.get_series_with_ids, new_ids, progress_callback)
            if response:
                with db.atomic():
    ...
    def sync_episodes(self, remote_ids, dialog):
        ...
        if new_ids:
            ...
            logging.debug("Found missing {0} of {1} episodes".format(new_ids_count, len(remote_ids)))
            response = self.do_chunked_request(api.get_episodes_with_ids, new_ids, progress_callback)
            if response:
                for episode in response:
                    try:
                        # We need to cope with the unique contraint for (series, season, number) 
                        ...
                        if utilities.if_sqlite_version_and_up((3,24,0)):
                            episode_id = (Episode
                                    .insert(episode)
                                    .on_conflict(
                                        conflict_target=[Episode.series, Episode.season, Episode.number],
    ...
    def sync_releases(self, remote_ids, dialog):
        ...
        if new_ids:
            ...
            logging.debug("Found missing {0} of {1} releases".format(new_ids_count, len(remote_ids)))
            response = self.do_chunked_request(api.get_releases_with_ids, new_ids, progress_callback)
            if response:
                with db.atomic():
    ...
    def do_chunked_request(self, handler, ids, progress_callback=None):
        ...
        for index, chunked_ids in enumerate(chunked(ids, REQUEST_CHUNK_SIZE)):
            if progress_callback: 
                ...
                progress_callback(percent, ids_count - index*REQUEST_CHUNK_SIZE)
            logging.debug("Requesting {0} of {1} chunks".format(index, ids_count // REQUEST_CHUNK_SIZE ))
            result.extend(self.do_request(lambda: handler(self.client_id, chunked_ids)))
        return result
